...
func reachabilityExplanations() map[string]Explanation {
	return map[string]Explanation{
		...
		DIP102: {
			...
			Example: "Router -> A when ctx.outcome = success\n# missing default edge from Router",
		},
		DIP103: {
			Code:    DIP103,
		...
		DIP104: {
			...
			Example: "node Retry { role agent }\nRetry -> Retry when ctx.outcome = fail  # no bound",
		},
		DIP105: {
			Code:    DIP105,
			...
			Example: "Start -> A when ctx.outcome = fail\nA -> End  # no success path",
		},
		DIP144: {
			Code:    DIP144,
...
func conditionExplanations() map[string]Explanation {
	return map[string]Explanation{
		DIP120: {
			...
			Example: "A -> B when status == done  # should be ctx.status",
		},
		DIP121: {
			Code:    DIP121,
			...
			Example: "node A { writes: result }\nA -> B when ctx.score = high  # score not in A's writes",
		},
		DIP122: {
			Code:    DIP122,
			...
			Example: "node T { role tool outputs: success, fail }\nT -> B when ctx.outcome = retry  # retry not declared",
		},
		DIP123: {
			Code:    DIP123,
