...
70 diagnostic codes: DIP001-DIP010 (structural errors), DIP101-DIP160 (semantic lint — mostly warnings; DIP155-DIP158 are error-severity and fail `lint`/`check`). DIP101/DIP102 suppress automatically when source node conditions are exhaustive (success/fail pairs, contains/not-contains complementary pairs). DIP121/DIP122 only fire when source nodes declare writes/outputs (advisory metadata).
