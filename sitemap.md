...
- [Configuration](https://dippin.org/configuration.html) — Reference for the defaults block and graph-level configuration: models, retry and recovery, budgets, context management, and tool safety.
- [Edges & Routing](https://dippin.org/edges.html) — How .dip workflows connect nodes: the edges block, edge attributes, outcome shorthand, loops, section-level defaults, conditions, and the routing cascade.
...
- [Export & Visualization](https://dippin.org/export.html) — Turn a Dippin workflow into a diagram — export to Mermaid or Graphviz DOT, migrate existing DOT pipelines to .dip, and pipe DOT output through Graphviz for PNG/SVG images.
...
- [Lint Rules](https://dippin.org/lint.html) — 60 semantic lint checks (DIP101–DIP160) that flag likely bugs and questionable patterns, grouped by concern. Run with dippin lint.
...
- [Prompts & Context](https://dippin.org/prompts.html) — How agent prompts are authored in .dip files: multiline text blocks, prompt fields, the defaults prompt cascade, context truncation, and interpolation.
...
- [Structural Validation](https://dippin.org/validation.html) — 10 structural error codes (DIP001–DIP010) a workflow must satisfy to execute. Caught by dippin validate before runtime.
