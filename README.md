...
| Validation | Silent — typos in attrs are ignored | Diagnostic codes (DIP001–DIP010, DIP101–DIP165) |
...
| `dippin lint <file>` | Validation + semantic warnings (DIP101–DIP165) |
...
### Warnings (DIP101–DIP165 — selected; see [docs/validation.md](docs/validation.md) for the full catalog)
...
| DIP163 | `writable_paths_mode` not exactly `require`/`prefer` (error — fails `lint`/`check`; no trimming or case-folding) |
| DIP164 | `writable_paths_mode` set without `writable_paths` (inert; hint) |
| DIP165 | `writable_paths_mode: prefer` runs UNJAILED on hosts without Landlock ABI v3 (hint) |
