...
### Added
- **Typed `writable_paths_mode` on agent nodes and parallel branch overrides, with DIP163–DIP165** ([#307](https://github.com/2389-research/dippin-lang/issues/307)). tracker's `writable_paths_mode: require | prefer` (default `require`) — under `prefer`, a *host-capability* jail refusal (no Landlock ABI v3: macOS, Linux < 6.2) degrades to an unjailed run with a recorded `jail_degraded` event, while authoring and backend refusals still refuse in both modes — used to ride the agent `params:` passthrough, so dippin could not lint it. `ir.AgentConfig.WritablePathsMode` / `ir.BranchConfig.WritablePathsMode` carry the value **verbatim** (no trim or case-fold — tracker fails closed on `Prefer` / `"prefer "`); absent stays `""` so `fmt` never invents the attribute, and a present-but-empty value is a parse error naming the node. **DIP163** (error severity — fails `lint`/`check`) rejects any value other than exactly `require` or `prefer`, quoting the offending value so a trailing space is visible; **DIP164** (hint) flags a mode set without `writable_paths` on the node or branch (for a branch: neither the branch nor the target agent declares a scope); **DIP165** (hint) reminds that a `prefer` node runs UNJAILED on hosts without Landlock and must never be described as sandboxed. **DIP133** now also covers `writable_paths` and `writable_paths_mode` under `params:`, so the old passthrough spelling is flagged instead of silently shadowing the typed field. Formatter emits the attribute right after `writable_paths`; DOT export, migrate, parity, LSP completion, EBNF and tmLanguage carry it. 75 diagnostic codes. New example `examples/agent_writable_paths.dip`.
- **`parser.ResolveFileDirectivesFS(w, fsys fs.FS, baseDir)`** ([#304](https://github.com/2389-research/dippin-lang/issues/304)). Resolves `prompt_file` / `system_prompt_file` / `command_file` / `prompt_include` and the defaults-block `*_file` cascade from any `fs.FS` — `embed.FS`, `fs.Sub`, `fstest.MapFS` — so a host that bundles a workflow and its sidecars in its binary no longer has to materialize the tree to a temp dir. One cascade traversal now serves both resolvers; `ResolveFileDirectives` keeps its signature and its full disk safety chain (absolute-path rejection, `EvalSymlinks` containment, O_NOFOLLOW single-fd open, fstat, 4 MiB cap) byte-for-byte and does **not** go through `os.DirFS`. The FS variant rejects absolute paths and any `..` segment on the raw directive path *before* joining, requires `fs.ValidPath` on the joined name, enforces the size cap by stat before a bounded read, rejects directories and non-regular files, and — on any FS that implements `fs.ReadLinkFS` (Go 1.25; `os.DirFS` does) — `Lstat`s every component below `baseDir` and rejects symlinks, so a symlinked parent or leaf cannot escape the root. An FS that cannot report symlinks (`embed.FS`, `MapFS`) also cannot contain them. Lstat-then-Open is not atomic on a live tree, so `ResolveFileDirectives` remains the race-hardened path for disk-backed workflows. Errors name only the user-written path, as on disk. A disk/FS parity test resolves every existing fixture both ways and deep-compares the workflows. tracker can delete its stopgap `pipeline.ResolveFileDirectivesFS` once it bumps the pin.
...
### Added
- **`openai-compat` is a first-class custom-gateway provider** ([#297](https://github.com/2389-research/dippin-lang/issues/297)). An org deploying behind a BYOK / OpenAI-compatible edge router serves model ids a static catalog cannot know, and DIP108 had no named escape hatch short of enumerating them with `--extra-models`. `prices.json` now carries a `custom_providers` block; a provider declared there has no model list, and every id under it is treated as known-but-unpriced — the same `Priced=false` state a Qwen entry carries. `pricing.LookupProvider("openai-compat", anything)` returns found with `Priced=false`, `pricing.KnownProvider` recognizes it, `openai-compatible` is a provider alias for it, and the new `pricing.CustomProvider(p)` / `pricing.CustomProviders()` let downstream consumers detect the case. DIP108 stays quiet for any model under `openai-compat`; `dippin cost` prices it at `$0` with an assumption that names the gateway instead of calling the id an "unknown model". The exemption is scoped to the declared name, so DIP108 stays strict for `openai`/`anthropic`/`gemini` typos and for undeclared gateway names. A `@` in an id under a custom provider is an opaque id, not a family alias — no DIP162, and `fmt` leaves it alone. New example `examples/openai_compat_gateway.dip`.
