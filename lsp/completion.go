...
func fieldCompletions() []protocol.CompletionItem {
	fields := []struct{ name, detail string }{
		...
		{"last_response_truncate:", "Char cap on the auto-injected previous response in this agent's prompt (chain-attack mitigation; 0/unset = no truncation; runtime-enforced)"},
	}
	var items []protocol.CompletionItem
