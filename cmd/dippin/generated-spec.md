...
| `tool_access` | string | LLM tool-catalog gate. Only one explicit value: `none` (no tools). Omitted = full catalog. Invalid values are fail-closed at runtime and warned by DIP139. Requires tracker `>= vX.Y.Z`. See "Agent Tool Access" below. |
...
**Agent Tool Access (`tool_access:`)** — *added v0.32.0; requires tracker `>= vX.Y.Z`.*
