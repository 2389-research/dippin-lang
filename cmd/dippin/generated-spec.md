...
75 diagnostic codes across two categories:
...
| DIP164 | `writable_paths_mode` set with no `writable_paths` to scope it — for an agent, neither it nor a parallel branch targeting it declares globs; for a branch, neither the branch nor its target agent does; the mode is inert (Hint) | Add `writable_paths: <globs>` on the node, a branch targeting it, or (branch mode) the branch/target agent — or remove the mode |
