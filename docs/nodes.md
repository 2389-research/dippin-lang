...
Block-form parallels export a `branches=` node attribute alongside the standard `targets=` attribute. Each branch is serialised as `;`-joined `key=value` tokens (`target` plus any of `model`/`provider`/`fidelity`), with branches joined by `,`:
