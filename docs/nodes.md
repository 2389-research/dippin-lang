...
| `writable_paths_mode` | `require` \| `prefer` | — (`require`) | How the runtime treats a **host-capability** jail refusal for `writable_paths`. `require` (default when absent): refuse to start on a host without Landlock ABI v3 (macOS, Linux < 6.2). `prefer`: run **UNJAILED** there with a recorded `jail_degraded` event, TUI/doctor warning and run-manifest entry — such a node is *not* sandboxed on those hosts and operator copy must not call it so. Authoring (malformed glob) and backend (`claude-code`/`acp`) refusals still refuse in both modes. Matched **exactly** — DIP163 (error) rejects `Prefer`, `"prefer "`, etc.; a bare `writable_paths_mode:` is a parse error. DIP164 hints when set without `writable_paths`; DIP165 hints on every `prefer`. Also a per-branch `parallel` override (omitted = inherit the target's mode). |
