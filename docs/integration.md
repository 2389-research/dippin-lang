...
**In-memory bundles.** `dipx.OpenReader(ctx, r io.ReaderAt, size int64) (*Bundle, error)` loads a bundle straight from bytes — the same strict admission, hash verification, and ref-walking as `Open`, but with no filesystem anchor. This is the path for an **embedded** consumer that bakes its `.dipx` into the binary via `go:embed`: `dipx.OpenReader(ctx, bytes.NewReader(embedded), int64(len(embedded)))` avoids materializing a temp file (and its `0600` window). Errors carry no bundle path (there is none on an in-memory source); wrap them yourself if you need one.
