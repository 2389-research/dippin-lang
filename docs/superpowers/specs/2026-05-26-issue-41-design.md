...
3. **Companion list fields (`disallowed_tools`, `allowed_tools`).** Round-1 finding #2 demonstrated typo footguns (lowercase silently no-ops against tracker's CamelCase catalog). Defer until a `KnownAgentTools` registry and case-normalization are locked. ([#55](https://github.com/2389-research/dippin-lang/issues/55))
...
8. **`Params` bypass lint.** Author writes `params: { allowed_tools: Bash }` to re-enable tools. v1 defends against this on the tracker side (see Tracker design below); a dippin lint for the params surface is deferred. ([#60](https://github.com/2389-research/dippin-lang/issues/60))
